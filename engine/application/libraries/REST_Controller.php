...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    public function __construct($config = 'rest')
    {
        ...
        $this->load->library('REST_Formatlib');
        ...
        if ($this->request->format and $this->request->body) {
            $this->request->body = $this->rest_formatlib->factory($this->request->body, $this->request->format)->to_array();
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if ( ! method_exists($this, $controller_method)) {
            if (config_item('service_not_found') && method_exists($this, config_item('service_not_found'))){
                $service_not_found = config_item('service_not_found');
                $this->$service_not_found();
            }else{
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
        ...
        // Otherwise (if no data but 200 provided) or some data, carry on camping!
            ...
            // If the format method exists, call and return the output in that format
            elseif (method_exists($this->rest_formatlib, 'to_'.$this->response->format)) {
                // Set the correct format header
                header('Content-Type: '.$this->_supported_formats[$this->response->format] . '; charset=' . strtolower($this->config->item('charset')));

                $output = $this->rest_formatlib->factory($data)->{'to_'.$this->response->format}();
            }
